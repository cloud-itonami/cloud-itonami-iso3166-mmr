...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MMR, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up annual-return-status-mismatch, FLAGSHIP) ==")
    ...
    (println "== filing/submit eng-4 (annual-return-status-mismatch -> HARD hold, FLAGSHIP) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up mic-permit-unverified) ==")
    ...
    (println "== filing/submit eng-5 (mic-permit-unverified -> HARD hold) ==")
