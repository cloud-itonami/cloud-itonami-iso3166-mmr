(ns marketentry.store
  "SSoT for the MMR market-entry compliance actor, behind a `Store`
  ...
  `DatomicStore` uses `langchain-store.core` (`ls/enc`/`ls/dec*`/
  `ls/read-stream`/`ls/append-blob!`) for the ledger/assessment
  EDN-blob codec and event-log read/append, instead of hand-rolling a
  private `enc`/`dec*` two-liner -- the shared seam ADR-2607141600
  documents (~190 cloud-itonami actors otherwise duplicate this),
  following the `cloud-itonami-iso3166-idn` sibling's convention.
...
(defn demo-data
  ...
  `:ar-due-date`/`:ar-filed-date`/`:claimed-ar-status` are ground truth
  for the flagship Annual Return staged-deadline check (DICA's own
  28-day late-fee grace window / 'Suspended' rule); `:requires-mic-
  permit?`/`:mic-permit-verified?` are ground truth for the conditional
  MIC permit check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Shwe Nadi Trading Co., Ltd." :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :ar-due-date "2026-06-01" :ar-filed-date "2026-05-28" :claimed-ar-status :current
             :requires-mic-permit? true :mic-permit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MMR" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :ar-due-date "2026-06-01" :ar-filed-date "2026-05-28" :claimed-ar-status :current
             :requires-mic-permit? true :mic-permit-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Golden Delta Logistics Co., Ltd." :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "MMR" :status :intake}
    "eng-4" {:id "eng-4" :operator "Ayeyar Systems Myanmar Co., Ltd." :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :ar-due-date "2026-01-01" :ar-filed-date "2026-09-01" :claimed-ar-status :current
             :requires-mic-permit? true :mic-permit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MMR" :status :intake}
    "eng-5" {:id "eng-5" :operator "Taninthayi Freight Co., Ltd." :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :ar-due-date "2026-06-01" :ar-filed-date "2026-05-28" :claimed-ar-status :current
             :requires-mic-permit? true :mic-permit-verified? false
             :drafted? false :submitted? false
             :jurisdiction "MMR" :status :intake}
    "eng-6" {:id "eng-6" :operator "Yadanar Civic Tech Co., Ltd." :procurement-channel "Myanmar Companies Online (MyCO) / MIC e-filing"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :ar-due-date "2026-06-01" :ar-filed-date "2026-06-10" :claimed-ar-status :late-fee-due
             :requires-mic-permit? true :mic-permit-verified? true
             :drafted? false :submitted? false
             :jurisdiction "MMR" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator procurement-channel base-fee monthly-rate monitoring-months claimed-fee
                               ar-due-date ar-filed-date claimed-ar-status
                               requires-mic-permit? mic-permit-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    ar-due-date                           (assoc :engagement/ar-due-date ar-due-date)
    ar-filed-date                         (assoc :engagement/ar-filed-date ar-filed-date)
    claimed-ar-status                     (assoc :engagement/claimed-ar-status claimed-ar-status)
    (some? requires-mic-permit?)          (assoc :engagement/requires-mic-permit? requires-mic-permit?)
    (some? mic-permit-verified?)          (assoc :engagement/mic-permit-verified? mic-permit-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/procurement-channel :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/ar-due-date :engagement/ar-filed-date :engagement/claimed-ar-status
   :engagement/requires-mic-permit? :engagement/mic-permit-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :procurement-channel (:engagement/procurement-channel m)
     ...
     :ar-due-date (:engagement/ar-due-date m) :ar-filed-date (:engagement/ar-filed-date m)
     :claimed-ar-status (:engagement/claimed-ar-status m)
     :requires-mic-permit? (boolean (:engagement/requires-mic-permit? m))
     :mic-permit-verified? (boolean (:engagement/mic-permit-verified? m))
