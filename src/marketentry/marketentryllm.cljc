(ns marketentry.marketentryllm
  ...
  the Republic of the Union of Myanmar (MMR) public-sector market-entry
  compliance actor.
  ...
  cited), never a committed record or a real MyCO Annual Return / MIC
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world MyCO Annual Return / MIC
  filing request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けMyCO年次申告/MIC提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "ar-due-date=" (:ar-due-date e)
                        " ar-filed-date=" (:ar-filed-date e)
                        " claimed-ar-status=" (:claimed-ar-status e)
                        " mic-permit-verified?=" (:mic-permit-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-mic-permit? e))
                              (:mic-permit-verified? e)))
                   0.9 0.3)}))
