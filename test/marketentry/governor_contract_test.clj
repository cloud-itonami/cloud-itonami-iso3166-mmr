  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Shwe Nadi Trading Co., Ltd."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Shwe Nadi Trading Co., Ltd." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest annual-return-status-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-ar-status does not match the independently recomputed staged-deadline tier -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:annual-return-status-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest mic-permit-unverified-is-held-and-unoverridable
  (testing "unverified MIC investment permit when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:mic-permit-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
