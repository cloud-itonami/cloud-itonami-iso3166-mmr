...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co., Ltd." :jurisdiction "MMR"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :ar-due-date "2026-06-01"
                                   :ar-filed-date "2026-05-28"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "MMR" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:ar-due-date (:engagement m)) (:ar-due-date (:engagement d))))
    (is (= (:claimed-ar-status (:engagement m)) (:claimed-ar-status (:engagement d))))
